...
int main() {
	...
	ImportNeumannFromCSV(isqfixed, qfixed, field, model_path + "Neumann0.csv");
    ...
    double voidlimit = 0.25;
    double perimeterlimit = 0.02;
    ...
    MMA<double> optimizer = MMA<double>(s0.size() + s1.size(), 3, 1.0,
		std::vector<double>(3, 0.0),
		std::vector<double>(3, 10000.0),
		std::vector<double>(3, 0.0), 
	...
	for(int k = 0; k < 400; k++){
        ...
        double l = 0.0;
        std::vector<double> dldr0 = std::vector<double>(s0.size(), 0.0);    //Sensitivities of perimeter
        std::vector<double> dldr1 = std::vector<double>(s1.size(), 0.0);    //Sensitivities of perimeter
        ...
        for (int i = 0; i < elements.size(); i++) {						
            ...
            for(auto j : neighbors[i]){
                ...
                dldr0[i] += (1.0 - r0[j]*r1[j])*(r0[i] - r0[j])/sqrt(pow(r0[i] - r0[j], 2.0) + perimetereps)/(double)neighbors[i].size()/(perimeterlimit*elements.size());     //  1st term
                dldr0[j] += ((1.0 - r0[i]*r1[i])*(r0[j] - r0[i])/sqrt(pow(r0[j] - r0[i], 2.0) + perimetereps) - r1[i]*sqrt(pow(r0[j] - r0[i], 2.0) + perimetereps))/(double)neighbors[j].size()/(perimeterlimit*elements.size());     //  2nd term
                dldr1[j] -= r0[i]*sqrt(pow(r0[j] - r0[i], 2.0) + perimetereps)/(double)neighbors[j].size()/(perimeterlimit*elements.size()); 
            }
		}
		g -= 1.0;
        h -= 1.0;
        ...
        std::vector<double> strainenergy = std::vector<double>(elements.size());
        ...
        AddElementScalers(strainenergy, "strainenergy", fout, false);
		fout.close();
		...
		//----------Check convergence----------
        std::cout << "Objective:\t" << f/scale0 << "\tWeight:\t" << g/scale1 << "\tvoid\t" << h/scale1 << "\tperimeter\t" << l/scale1 << "\t";
        ...
        std::vector<double> dlds = std::vector<double>(s0.size() + s1.size());
        for(int i = 0; i < elements.size(); i++){
            s[2*i] = s0[i];
            s[2*i + 1] = s1[i];
            dfds[2*i] = dfds0[i];
		...
		optimizer.UpdateVariables(s, f, dfds, { g, h, l }, { dgds, dhds, dlds });	
        for(int i = 0; i < elements.size(); i++){
            s0[i] = s[2*i];
            s1[i] = s[2*i + 1];
        }
