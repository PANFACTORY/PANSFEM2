...
int main() {
	...
	for(int k = 0; k < 500; k++){
        ...
        for(auto edge : edges) {
            std::vector<std::vector<std::pair<int, int> > > nodetoelement;
            Vector<double> Fe0;
            PlaneStrainSurfaceForce<double, ShapeFunction2Line, Gauss1Line>(Fe0, nodetoelement, edge, { 0, 1 }, x, [=](Vector<double> _x){
                Vector<double> f = { 0.0, 0.0 };
                if(fabs(_x(1) - 100.0) < 1.0e-5) {
                    f(1) = -exp(-0.5*pow((_x(0) - mubar)/sigma, 2.0))/(sqrt(2.0*M_PI)*sigma);
                }
                return f;
            }, 1.0);
            Assembling(F0, Fe0, nodetoglobal, nodetoelement, edge);
            Vector<double> Fe1;
            PlaneStrainSurfaceForce<double, ShapeFunction2Line, Gauss1Line>(Fe1, nodetoelement, edge, { 0, 1 }, x, [=](Vector<double> _x){
                Vector<double> f = { 0.0, 0.0 };
                if(fabs(_x(1) - 100.0) < 1.0e-5) {
                    f(1) = -(_x(0) - mubar)*exp(-0.5*pow((_x(0) - mubar)/sigma, 2.0))/(sqrt(2.0*M_PI)*pow(sigma, 3.0));
                }
                return f;
            }, 1.0);
            Assembling(F1, Fe1, nodetoglobal, nodetoelement, edge);
            Vector<double> Fe2;
            PlaneStrainSurfaceForce<double, ShapeFunction2Line, Gauss1Line>(Fe2, nodetoelement, edge, { 0, 1 }, x, [=](Vector<double> _x){
                Vector<double> f = { 0.0, 0.0 };
                if(fabs(_x(1) - 100.0) < 1.0e-5) {
                    f(1) = -(pow((_x(0) - mubar)/sigma, 2.0) - 1.0)*exp(-0.5*pow((_x(0) - mubar)/sigma, 2.0))/(sqrt(2.0*M_PI)*pow(sigma, 3.0));
                }
                return f;
            }, 1.0);
            Assembling(F2, Fe2, nodetoglobal, nodetoelement, edge);
        }
