...
int main() {
	...
	for(int k = 0; k < 200; k++){
		...
		std::vector<std::vector<double> > dcompliances = std::vector<std::vector<double> >(n, std::vector<double>(s.size()));
		for (int i = 0; i < elements.size(); i++) {
			Matrix<double> Ke;
			...
			for(int l = 0; l < n; l++){
				...
				compliances[l] += (E0*(1.0 - pow(s[2*i], p)) + (E1*(1.0 - pow(s[2*i + 1], q)) + E2*pow(s[2*i + 1], q))*pow(s[2*i], p))*ueKeue;
				dcompliances[l][2*i] = p*(-E0 + (E1*(1.0 - pow(s[2*i + 1], q)) + E2*pow(s[2*i + 1], q)))*pow(s[2*i], p - 1.0)*ueKeue;
                dcompliances[l][2*i + 1] = q*(-E1 + E2)*pow(s[2*i + 1], q - 1.0)*pow(s[2*i], p)*ueKeue;
			}
