...
int main() {
	...
	std::vector<Vector<double> > u = std::vector<Vector<double> >(x.size(), Vector<double>(2));		//	Node displacement vector array
	std::vector<Vector<double> > r = std::vector<Vector<double> >(x.size(), Vector<double>(2));		//	Node reaction force vector array 
	...
	for(auto element : elements) {
		...
		Matrix<double> Ke;
		PlaneStrainStiffness<double, ShapeFunction3Triangle, Gauss1Triangle>(Ke, nodetoelement, element, { 0, 1 }, x, 210000.0, 0.3, 1.0);
		Vector<double> Keue = Ke*ElementVector(u, nodetoelement, element);
		Assembling(R, Keue, nodetoglobal, nodetoelement, element);
	}

	Disassembling(r, R, nodetoglobal);
