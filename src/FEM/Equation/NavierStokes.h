...
namespace PANSFEM2 {
	...
	//******************************Get element tengent matrix for Navier-Stokes equation with PSPG/SUPG******************************
	template<class T, template<class>class SFU, template<class>class SFP, template<class>class IC>
	void NavierStokesPSPGSUPGTangent(Matrix<T>& _Ke, Vector<T>& _Fe, std::vector<std::vector<std::pair<int, int> > >& _nodetoelementu, const std::vector<int>& _elementu, std::vector<std::vector<std::pair<int, int> > >& _nodetoelementp, const std::vector<int>& _elementp, const std::vector<int>& _doulist, std::vector<Vector<T> >& _x, std::vector<Vector<T> >& _up, T _rho, T _mu) {
		assert(_doulist.size() == 3);
