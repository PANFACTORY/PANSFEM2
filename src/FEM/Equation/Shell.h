...
namespace PANSFEM2 {
	...
	void LinearIsotropicElasticShell(Matrix<T>& _Ke, std::vector<Vector<T> >& _x, std::vector<int>& _element, std::vector<Vector<T> >& _v, T _E, T _V, T _t) {
		...
		for (int g = 0; g < IC<T>::N; g++) {
            ...
            Vector<T> gausspoint = IC<T>::Points[g];
            Vector<T> N = SF<T>::N(gausspoint.Segment(0, 2));
			Matrix<T> dNdr = SF<T>::dNdr(gausspoint.Segment(0, 2));
            
			//----------Generate Jacobi matrix and derivative----------
            T zeta = gausspoint.Segment(2, 3)(0);
            ...
            _Ke += B.Transpose()*D*B*detJ*IC<T>::Weights[g][0]*IC<T>::Weights[g][1]*IC<T>::Weights[g][2];
		}
	}
}