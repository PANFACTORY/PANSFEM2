...
namespace PANSFEM2 {
	...
	template<class T>
	void LinearIsotropicElasticSolid(Matrix<T>& _Ke, std::vector<std::vector<T> >& _x, std::vector<int>& _element, T _E, T _V) {
	...
	template<class T>
	void TotalLagrangeSolid(Matrix<T>& _Ke, std::vector<T>& _Fe, std::vector<std::vector<T> >& _x, std::vector<std::vector<T> >& _u, std::vector<int>& _element, T _E, T _V) {
		...
		for (int g = 0; g < 8; g++) {
			...
			Matrix<T> E = 0.5 * (Z + Transpose(Z) + Transpose(Z)*Z);
			...
			I(1, 0)  = 1.0;	I(1, 1) = 1.0;	I(1, 2) = 0.0;
			I(2, 0)  = 1.0;	I(2, 1) = 0.0;	I(2, 2) = 1.0;
			Matrix<T> F = I + Z;
