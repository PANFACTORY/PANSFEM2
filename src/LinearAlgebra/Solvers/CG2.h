...
namespace PANSFEM2 {
    ...
    template<class T>
    void CG(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20) {
    ...
    template<class T>
    void BiCGSTAB(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
    ...
    template<class T>
    void BiCGSTAB2(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
    ...
    template<class T>
    void GPBiCG(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
    ...
    //  Solve [A]{x} = {b}, [A] is a diagonal matrix
    template<class T>
    inline void DiagonalOrdering(T *_A, T *_x, T *_b, int _n, T _eps = 1.0e-20) {
        assert(0 < _n && T() < _eps);
        for (int i = 0; i < _n; i++) {
            _x[i] = _eps < fabs(_A[i]) ? _b[i]/_A[i] : _b[i];
        }
    }

    //********************Scaling preconditioned CG solver for dense matrix********************
    template<class T>
    void ScalingCG(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20, T _scalingmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
        ...
        for (; k < _itrmax; ++k) {
            ...
            xeaypbz(_x, 1.0, _x, alpha, p, _n);
            xeaypbz(r, 1.0, r, -alpha, Ap, _n);
            DiagonalOrdering(D, Mr, r, _n, _scalingmin);
    ...
    template<class T>
    void ScalingBiCGSTAB(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20, T _scalingmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
        ...
        int k = 0;
        for (; k < _itrmax; ++k) {
            DiagonalOrdering(D, Mp, p, _n, _scalingmin);
            for (int i = 0; i < _n; ++i) {
                AMp[i] = T();
                for (int j = 0; j < _n; ++j) {
            ...
            DiagonalOrdering(D, Ms, s, _n, _scalingmin);
            for (int i = 0; i < _n; ++i) {
                AMs[i] = T();
                for (int j = 0; j < _n; ++j) {
    ...
    template<class T>
    void ScalingBiCGSTAB2(T *_A, T *_x, T *_b, int _n, int _itrmax, T _eps, bool _isdebug = false, T _bbmin = 1.0e-20, T _scalingmin = 1.0e-20) {
        assert(0 < _n && 0 < _itrmax && T() < _eps);
        ...
        for(; k < _itrmax; k++) {
            ...
            weaxpbypcz(z, ita, z, omega, r, -alpha, u, _n);
            DiagonalOrdering(D, Mz, z, _n, _scalingmin);
