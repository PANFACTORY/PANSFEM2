...
public:
        ...
        std::vector<Vector<T> > GenerateNodes2();
        std::vector<std::vector<int> > GenerateElements();
        std::vector<std::vector<int> > GenerateElements2();
        template<class F>
...
private:
    ...
    template<class T>
    std::vector<Vector<T> > SquareMesh<T>::GenerateNodes2(){
        std::vector<Vector<T> > nodes = std::vector<Vector<T> >((2.0*this->nx + 1)*(2.0*this->ny + 1) - this->nx*this->ny);
        //----------Corner nodes----------
        for(int i = 0; i < this->nx + 1; i++) {
            for(int j = 0; j < this->ny + 1; j++) {
                nodes[(this->ny + 1)*i + j] = { this->x*(i/(T)this->nx), this->y*(j/(T)this->ny) };
        ...
        for(int i = 0; i < this->nx + 1; i++) {
            for(int j = 0; j < this->ny; j++) {
                nodes[this->ny*i + j + (this->nx + 1)*(this->ny + 1)] = { this->x*(i/(T)this->nx), this->y*((j + 0.5)/(T)this->ny) };
            } 
        }
        for(int i = 0; i < this->nx; i++) {
            for(int j = 0; j < this->ny + 1; j++) {
                nodes[(this->ny + 1)*i + j + (this->nx + 1)*(2.0*this->ny + 1)] = { this->x*((i + 0.5)/(T)this->nx), this->y*(j/(T)this->ny) };
            }
        }
        return nodes;
    }
    ...
    template<class T>
    std::vector<std::vector<int> > SquareMesh<T>::GenerateElements2(){
        std::vector<std::vector<int> > elements = std::vector<std::vector<int> >(this->nx*this->ny);
        for(int i = 0; i < this->nx; i++){
            for(int j = 0; j < this->ny; j++){
                elements[this->ny*i + j] = { (this->ny + 1)*i + j, (this->ny + 1)*(i + 1) + j, (this->ny + 1)*(i + 1) + (j + 1), (this->ny + 1)*i + (j + 1), (this->ny + 1)*i + j + (this->nx + 1)*(2*this->ny + 1), this->ny*(i + 1) + j + (this->nx + 1)*(this->ny + 1), (this->ny + 1)*i + (j + 1) + (this->nx + 1)*(2*this->ny + 1), this->ny*i + j + (this->nx + 1)*(this->ny + 1) };
            }
        }
        return elements;
    }
    ...
    template<class T>
    std::vector<std::vector<int> > SquareMesh<T>::GenerateEdges2() {
        std::vector<std::vector<int> > edges = std::vector<std::vector<int> >(2*(this->nx + this->ny));
        for(int i = 0; i < this->nx; i++) {
            edges[i] = { (this->ny + 1)*i, (this->ny + 1)*(i + 1), (this->ny + 1)*i + (this->nx + 1)*(2*this->ny + 1) };
            edges[2*this->nx + this->ny - i - 1] = { (this->ny + 1)*(i + 1) + this->ny, (this->ny + 1)*i  + this->ny, (this->ny + 1)*i + (this->nx + 1)*(2*this->ny + 1) + this->ny };
        }
        for(int j = 0; j < this->ny; j++) {
            edges[j + this->nx] = { (this->ny + 1)*this->nx + j, (this->ny + 1)*this->nx + j + 1, j + (this->nx + 1)*(this->ny + 1) + this->nx*this->ny };
    ...
    template<class T>
    template<class F>
    std::vector<std::pair<std::pair<int, int>, T> > SquareMesh<T>::GenerateFixedlist2(std::vector<int> _ulist, F _iscorrespond) {
        assert(0 <= *std::min_element(_ulist.begin(), _ulist.end()));
        std::vector<std::pair<std::pair<int, int>, T> > ufixed;
        //----------Corner nodes----------
        for(int i = 0; i < this->nx + 1; i++){
        ...
        for(int i = 0; i < this->nx + 1; i++) {
            for(int j = 0; j < this->ny; j++) {
                if(_iscorrespond(Vector<T>({ this->x*(i/(T)this->nx), this->y*((j + 0.5)/(T)this->ny) }))) {
                    for(auto ui : _ulist) {
                        ufixed.push_back({ { this->ny*i + j + (this->nx + 1)*(this->ny + 1), ui }, T() });
                    }
                }
            } 
        }
        for(int i = 0; i < this->nx; i++) {
            for(int j = 0; j < this->ny + 1; j++) {
                if(_iscorrespond(Vector<T>({ this->x*((i + 0.5)/(T)this->nx), this->y*(j/(T)this->ny) }))) {
                    for(auto ui : _ulist) {
                        ufixed.push_back({ { (this->ny + 1)*i + j + (this->nx + 1)*(2.0*this->ny + 1), ui }, T() });
                    }
                }
            }
        }
